...
exports.getParquesNacionales = async (req, res) => {
...
exports.getParqueNacional = async (req, res) => {
    ...
    console.log("SELECT * FROM Parques where Id=" + Id + ";");

    try {
        await pool.connect(); // Abrir conexión
        const result = await pool.query("SELECT * FROM Parques where Id=" + Id + ";");
        await pool.close(); // Cerrar conexión
...
exports.updateParqueNacional = async (req, res) => {
    ...
    console.log("EXEC sp_editPark " + Id + " ,'" + Nombre + "','" + Area_de_Conservacion + "','" + Provincia + "'," + Tarifa_Nacionales_colones + "," +
